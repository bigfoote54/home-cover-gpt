...
function fixUnescapedEntities(content) {
  // Fix single quotes - replace ' with &apos;
  content = content.replace(/(?<!&)(?<!&#)(?<!&#x)'/g, '&apos;');
  
  // Fix double quotes - replace " with &quot;
  // But be careful not to break JSX attributes
...
function fixHtmlLinks(content) {
  ...
  if (!content.includes("import Link from 'next/link'") && !content.includes('import Link from "next/link"')) {
    // Find the last import statement and add Link import after it
...
function main() {
  ...
  if (fixedCount > 0) {
    ...
    console.log('   Make sure you are in the correct repository branch.');
  }
}
