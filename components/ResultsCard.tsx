...
const ResultsCard = ({ coverage, gaps, recommendations, onExportReport }: ResultsCardProps) => {
  return (
              ...
              {gaps.map((gap, index) => (
                ...
                >
                  <AlertCircle className={`h-4 w-4 ${
                    gap.type === "warning" ? "text-warning" : "text-destructive"
                  }`} />
                  <AlertDescription>
                    <div className={`font-medium ${
                      gap.type === "warning" ? "text-warning-foreground" : "text-destructive-foreground"
                    }`}>
                      {gap.title}
                    </div>
                    <div className="text-sm text-muted-foreground mt-1">{gap.description}</div>
                  </AlertDescription>
                </Alert>
              ))}
            </CardContent>
